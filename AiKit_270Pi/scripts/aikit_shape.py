...
offset_path = os.path.expanduser('~/convenient_aikit/AiKit_UI/libraries/offset/mechArm 270 for Pi_shape.txt')
...
class Object_detect():
    ...
    def __init__(self, camera_x=camera_x, camera_y=camera_y, camera_z=camera_z):
    ...
    def transform_frame(self, frame):
        ...
        if self.x1 != self.x2:
            ...
            frame = frame[int(self.y2 * 0.66):int(self.y1 * 1.11),
                    int(self.x1 * 0.8):int(self.x2 * 1.11)]
        return frame
